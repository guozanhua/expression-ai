...
def read_data(data_dir, force=False):
  ...
  if force or not os.path.exists(pickle_file):
    ...
    train_images = np.vstack(
        data_frame['Pixels']).reshape(-1, IMAGE_SIZE, IMAGE_SIZE, 1)
    print(train_images.shape)
    train_labels = np.array([i for i in map(create_onehot_label, data_frame[
                            'Emotion'].values)]).reshape(-1, NUM_LABELS)
...
tf.flags.DEFINE_string("logs_dir", "EmotionDetector_logs/",
                       "Path to where log files are to be saved")
tf.flags.DEFINE_string("mode", "train", "model: train (DEFAULT)/ test")

train_images, train_labels, valid_images, valid_labels, test_images = read_data(
    FLAGS.data_dir)
...
open('emotion_recog.json', 'w').write(model_json)
model.save_weights('emotion_recog_weights.h5', overwrite=True)
